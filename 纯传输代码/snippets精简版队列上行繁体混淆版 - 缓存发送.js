...
const 手動資料管線 = async (可讀流, 可寫通道, 關閉連線) => {
    const 安全緩衝區大小 = 緩衝區大小 - 最大區塊長度, 半區塊長度 = 最大區塊長度 >> 1, 直送緩衝區大小 = 半區塊長度 * 3, 快速刷新偏移量 = Math.max((緩衝區大小 / 刷新時間) << 1, 最大區塊長度 << 1);;
    let 緩衝區, 緩衝區視圖, 備用緩衝區 = new ArrayBuffer(最大區塊長度);
    let 偏移量 = 0, 總位元組 = 0, 計時器識別 = null, 恢復函式 = null, 正在讀取 = false, 需要刷新 = false, 保護刷新 = false;
    let 直送緩衝區 = new Uint8Array(直送緩衝區大小), 直送偏移量 = 0, 直送計時器 = null, 已關閉 = false, 快速刷新 = true;
    let 全域計數 = new Uint32Array(14), 全域位元組 = new Uint32Array(14);
    let 統計計數 = 0, 總計數 = 0, 全域總位元組 = 0;
    const 更新區塊統計 = (區塊長度, 衰減) => {
    ...
    const 刷新直送 = () => {
        直送計時器 && (clearTimeout(直送計時器), 直送計時器 = null);
        if (直送偏移量 > 0 && !已關閉) {
            直送偏移量 === 直送緩衝區大小
                ? (可寫通道.send(直送緩衝區), 直送緩衝區 = new Uint8Array(直送緩衝區大小))
                : 可寫通道.send(直送緩衝區.slice(0, 直送偏移量));
        }
        直送偏移量 = 0;
    };
    const 刷新輸出 = () => {
        ...
        if (偏移量 > 0 && !已關閉) {
            偏移量 > 安全緩衝區大小
                ? (可寫通道.send(緩衝區視圖.subarray(0, 偏移量)), 快速刷新 || (緩衝區 = new ArrayBuffer(緩衝區大小), 緩衝區視圖 = new Uint8Array(緩衝區)))
                : 可寫通道.send(緩衝區視圖.slice(0, 偏移量));
            偏移量 = 0;
        }
        快速刷新 && (總位元組 = 0, 直送緩衝區 ||= new Uint8Array(直送緩衝區大小), 緩衝區 = null, 緩衝區視圖 = null);
    ...
    try {
        while (true) {
            if (快速刷新) {
                const {done: 是否完成, value: 讀取值} = await 讀取器.read(new Uint8Array(備用緩衝區));
                if (是否完成) break;
                const 區塊長度 = 讀取值.byteLength;
                if (!區塊長度) continue;
                if (區塊長度 >= 半區塊長度) {
                    刷新直送(), 可寫通道.send(讀取值), 備用緩衝區 = new ArrayBuffer(最大區塊長度);
                } else if (直送偏移量 + 區塊長度 > 直送緩衝區大小) {
                    刷新直送(), 直送緩衝區.set(讀取值, 0), 直送偏移量 = 區塊長度, 直送計時器 = setTimeout(刷新直送, 1), 備用緩衝區 = 讀取值.buffer;
                } else {
                    直送緩衝區.set(讀取值, 直送偏移量), 直送偏移量 += 區塊長度, 直送計時器 ||= setTimeout(刷新直送, 1), 備用緩衝區 = 讀取值.buffer;
                }
                const 最大索引 = 更新區塊統計(區塊長度, false);
                if (區塊長度 < 下限陣列[最大索引]) {
                    總位元組 = 0;
                } else if ((總位元組 += 區塊長度) > 啟動閾值) {
                    刷新直送(), 快速刷新 = false, 緩衝區 = new ArrayBuffer(緩衝區大小), 緩衝區視圖 = new Uint8Array(緩衝區), 直送緩衝區 = null;
                }
            } else {
                ...
                if (需要刷新) {
                    ...
                    const 最大索引 = 更新區塊統計(區塊長度, true);
                    if (區塊長度 < 下限陣列[最大索引]) {
                        刷新輸出(), 快速刷新 = true, 總位元組 = 0, 直送緩衝區 = new Uint8Array(直送緩衝區大小), 緩衝區 = null, 緩衝區視圖 = null;
                    } else {
                        計時器識別 ||= setTimeout(刷新輸出, 刷新時間), 保護刷新 = 區塊長度 < 最大區塊長度;
                        偏移量 > 安全緩衝區大小 && await new Promise(結果值 => 恢復函式 = 結果值);
                    }
                }
            }
        }
    } catch {關閉連線?.(), 已關閉 = true} finally {正在讀取 = false, 刷新直送(), 刷新輸出()}
};
