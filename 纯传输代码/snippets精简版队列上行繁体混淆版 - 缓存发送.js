...
const 建立緩衝傳輸控制寫入器 = (寫入函式, 關閉連線) => {
    let 緩衝區 = new Uint8Array(最大區塊長度), 偏移量 = 0, 計時器識別 = null, 已關閉 = false;
    const 寫入失敗處理 = () => {
        已關閉 = true;
        關閉連線?.();
    };
    const 寫入 = (區塊值) => {
        if (已關閉) return;
        寫入函式.write(區塊值).catch(寫入失敗處理);
    };
    const 刷新 = () => {
        計時器識別 && (clearTimeout(計時器識別), 計時器識別 = null);
        if (已關閉) return 偏移量 = 0;
        if (!偏移量) return;
        寫入(緩衝區.subarray(0, 偏移量));
        偏移量 = 0;
    };
    return (區塊值) => {
        if (已關閉 || !區塊值?.byteLength) return;
        const 資料 = 區塊值 instanceof Uint8Array ? 區塊值 : new Uint8Array(區塊值);
        const 長度 = 資料.byteLength;
        if (長度 > (最大區塊長度 >> 1)) {
            刷新();
            return 寫入(資料);
        }
        const 可用長度 = 最大區塊長度 - 偏移量;
        if (長度 >= 可用長度) {
            緩衝區.set(資料.subarray(0, 可用長度), 偏移量);
            刷新();
            const 剩餘長度 = 長度 - 可用長度;
            if (剩餘長度 > 0) {
                緩衝區.set(資料.subarray(可用長度), 0);
                偏移量 = 剩餘長度;
            }
        } else {
            緩衝區.set(資料, 偏移量);
